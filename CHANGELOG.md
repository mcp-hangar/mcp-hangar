...
- **core:** a restart no longer drops the L7 egress policy the operator pushed to
  ...
  warning. See the upgrade note for which deployments keep the policy. ([#1560](https://github.com/mcp-hangar/mcp-hangar/pull/1560))
...
- **core:** the two error producers #1495 left behind now answer the one shape.
  ...
  `type` again. See `UPGRADE.md`. ([#1511](https://github.com/mcp-hangar/mcp-hangar/pull/1511))
- **core:** a server loaded with `hangar_load` now has a log buffer, and unloading
  ...
  it. ([#1510](https://github.com/mcp-hangar/mcp-hangar/pull/1510))
- **core:** a batch no longer leaves its routing headers bound on the caller's
  ...
  was bound before it. ([#1508](https://github.com/mcp-hangar/mcp-hangar/pull/1508))
- **core:** a configuration reload now gives a server it adds a log buffer.
  ...
  over. ([#1505](https://github.com/mcp-hangar/mcp-hangar/pull/1505))
- **core:** a tool call that names a group now asks a human for the
  ...
  configured. ([#1504](https://github.com/mcp-hangar/mcp-hangar/pull/1504))
- **core:** a configuration reload no longer drops an L7 egress policy set
  ...
  rebuilt server's output still reaches `hangar_logs`. ([#1500](https://github.com/mcp-hangar/mcp-hangar/pull/1500))
- **core:** a rate-limit refusal has one shape on every MCP tool path. A refusal
  ...
  `UPGRADE.md`. ([#1507](https://github.com/mcp-hangar/mcp-hangar/pull/1507))
- **core:** a front door now forwards the caller's tasks declaration upstream, so
  ...
  has a declaration forwarded on its behalf. ([#1501](https://github.com/mcp-hangar/mcp-hangar/pull/1501))
- **core:** a tool call made while a configuration reload swaps the servers and
  ...
  that decision too. ([#1497](https://github.com/mcp-hangar/mcp-hangar/pull/1497))
- **core:** the command-bus rate limit admits what it is configured to. A bucket
  ...
  `check_rate_limit()` is removed. See `UPGRADE.md`. ([#1494](https://github.com/mcp-hangar/mcp-hangar/pull/1494))
...
- **core:** three more paths no longer repeat an upstream's text. When an
  ...
  call it made. ([#1493](https://github.com/mcp-hangar/mcp-hangar/pull/1493))
...
- **core:** an upstream task in the current flat shape (`resultType: "task"`) is
  ...
  not a task that `tasks/*` would refuse it. See `UPGRADE.md`. ([#1491](https://github.com/mcp-hangar/mcp-hangar/pull/1491))
- **core:** the front door's per-call log line now says what the caller got. A
  ...
  answer. See `UPGRADE.md`. ([#1517](https://github.com/mcp-hangar/mcp-hangar/pull/1517))
...
- **core:** the front door now measures the surface it projects. It records how
  ...
  `tools/call` is not. ([#1406](https://github.com/mcp-hangar/mcp-hangar/pull/1406))
- **core:** in `front_door`, a `tools/call` to a name that has left the caller's
  ...
  error. ([#1395](https://github.com/mcp-hangar/mcp-hangar/pull/1395))
- **core:** the front door's flat projection is now generated by one explicit
  ...
  serves what it returns, so nothing a client sees changes. ([#1381](https://github.com/mcp-hangar/mcp-hangar/pull/1381))
- **core:** every call on the front door's flat projected surface now writes one
  ...
  not exist. No arguments are logged. ([#1396](https://github.com/mcp-hangar/mcp-hangar/pull/1396))
- **core:** new gauge `mcp_hangar_group_circuit_open{group}`. It reads 1 while
  ...
  `hangar_group_list` on the same replica ([#1408](https://github.com/mcp-hangar/mcp-hangar/pull/1408))
...
- **core:** a server Hangar gives up on now reads `dead` (4) on
  ...
  `UPGRADE.md` says what to use instead. ([#1399](https://github.com/mcp-hangar/mcp-hangar/pull/1399))
...
- **core:** `tool_access.rules` is no longer an accepted config key. The schema
  ...
  member. See `UPGRADE.md` ([#1433](https://github.com/mcp-hangar/mcp-hangar/pull/1433))
- **core:** a group's `circuit_breaker.reset_timeout_s` is removed. It never had
  ...
  `McpServerGroup` no longer accepts `circuit_reset_timeout_s`. See `UPGRADE.md` ([#1413](https://github.com/mcp-hangar/mcp-hangar/pull/1413))
- **core:** a gateway no longer saves its groups' circuit breakers to the saga
  ...
  `circuit_breaker_save_failed` are gone; `circuit_breaker_restored` never fired. ([#1414](https://github.com/mcp-hangar/mcp-hangar/pull/1414))
...
- **core:** a tool call held for approval no longer leaves an event loop, an idle
  ...
  as before. ([#1455](https://github.com/mcp-hangar/mcp-hangar/pull/1455))
- **core:** an in-memory store built on `SQLiteConnectionFactory` -- the
  ...
  and are unchanged. ([#1448](https://github.com/mcp-hangar/mcp-hangar/pull/1448))
- **core:** an in-memory SQLite event store (`SQLiteEventStore(":memory:")`) no
  ...
  are unchanged. ([#1441](https://github.com/mcp-hangar/mcp-hangar/pull/1441))
- **core:** a front door's flat `tools/call` now runs the configured
  ...
  refused with the same error on both paths. ([#1427](https://github.com/mcp-hangar/mcp-hangar/pull/1427))
- **core:** a configuration reload now applies the whole configuration and keeps
  ...
  409 and changes nothing: the mode needs a restart. See `UPGRADE.md`. ([#1430](https://github.com/mcp-hangar/mcp-hangar/pull/1430))
- **core:** a remote server and discovery built with `HangarConfig` now take
  ...
  `to_dict()` no longer includes the facade's `max_concurrency`. ([#1434](https://github.com/mcp-hangar/mcp-hangar/pull/1434))
- **core:** a configuration passed to `bootstrap(config_dict=...)` is now applied
  ...
  `config_dict`, is also refused. See `UPGRADE.md` ([#1421](https://github.com/mcp-hangar/mcp-hangar/pull/1421))
- **core:** the recovery saga's restarts now run with default settings. The saga
  ...
  after it. ([#1419](https://github.com/mcp-hangar/mcp-hangar/pull/1419))
- **core:** an aggregate no longer drops a domain event that another thread
  ...
  so none is lost. ([#1402](https://github.com/mcp-hangar/mcp-hangar/pull/1402))
- **core:** a task an upstream creates in answer to a front door's flat
  ...
  be recorded, the call fails the same way it does on `hangar_call`. ([#1403](https://github.com/mcp-hangar/mcp-hangar/pull/1403))
- **core:** a group's circuit breaker now opens after
  ...
  rotation ([#1397](https://github.com/mcp-hangar/mcp-hangar/pull/1397))
- **core:** shutdown now cancels the commands that sagas scheduled on a timer.
  ...
  at once, not up to one interval later. ([#1407](https://github.com/mcp-hangar/mcp-hangar/pull/1407))
- **core:** `hangar_status` answered with one replica's view in a shape that read
  ...
  fleet-wide view, query the per-replica metrics in Prometheus. ([#1391](https://github.com/mcp-hangar/mcp-hangar/pull/1391))
- **core:** the `hangar_status` dashboard drew a group in the server table, so a
  ...
  `hangar_details` are unchanged: they return each vocabulary as it is. ([#1412](https://github.com/mcp-hangar/mcp-hangar/pull/1412))
- **core:** a group's `healthy_count` counts the members that are `ready` and in
  ...
  `healthy_count: 0` until one does. `UPGRADE.md` says what to read instead. ([#1417](https://github.com/mcp-hangar/mcp-hangar/pull/1417))
- **core:** a passing health check now returns a group member to rotation. A
  ...
  back in rotation ([#1383](https://github.com/mcp-hangar/mcp-hangar/pull/1383))
- **cli:** `mcp-hangar pin` now honours `--quiet`, `MCP_LOG_LEVEL` and the
  ...
  fall back to polling, which is the only time it applies. ([#1385](https://github.com/mcp-hangar/mcp-hangar/pull/1385))
