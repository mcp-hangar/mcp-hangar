...
def test_granular_permission_constants_exist() -> None:
    assert PERMISSION_PROVIDERS_READ.resource_type == "mcp_servers"
    assert PERMISSION_PROVIDERS_READ.action == "read"
    assert PERMISSION_PROVIDERS_WRITE.resource_type == "mcp_servers"
    assert PERMISSION_PROVIDERS_WRITE.action == "write"
    assert PERMISSION_PROVIDERS_LIFECYCLE.resource_type == "mcp_servers"
...
def test_admin_role_includes_all_granular_permissions() -> None:
    assert ROLE_ADMIN.has_permission("mcp_servers", "read")
    assert ROLE_ADMIN.has_permission("mcp_servers", "write")
    assert ROLE_ADMIN.has_permission("mcp_servers", "lifecycle")
...
def test_developer_role_has_provider_read_write_and_lifecycle() -> None:
    assert ROLE_DEVELOPER.has_permission("mcp_servers", "read")
    assert ROLE_DEVELOPER.has_permission("mcp_servers", "write")
    assert ROLE_DEVELOPER.has_permission("mcp_servers", "lifecycle")
