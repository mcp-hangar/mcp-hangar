...
class TestCapabilityViolationDetected:
    def test_creates_with_required_fields(self) -> None:
        event = CapabilityViolationDetected(mcp_server_id="math", violation_type="egress_undeclared",
    ...
    def test_with_destination(self) -> None:
        event = CapabilityViolationDetected(mcp_server_id="fetch", violation_type="egress_undeclared",
    ...
    def test_has_event_id(self) -> None:
        event = CapabilityViolationDetected(mcp_server_id="p", violation_type="tool_schema_drift",
...
class TestEgressBlocked:
    def test_creates_with_defaults(self) -> None:
        event = EgressBlocked(mcp_server_id="code-interpreter", destination_host="exfil.attacker.com",
    ...
    def test_custom_enforcement_source(self) -> None:
        event = EgressBlocked(mcp_server_id="p", destination_host="192.168.1.1",
        destination_port=22,
        protocol="tcp",
...
class TestProviderCapabilityQuarantined:
    def test_creates_with_reason(self) -> None:
        event = ProviderCapabilityQuarantined(mcp_server_id="rogue-provider", reason="3 egress violations in 60s",
        violation_count=3,)
...
class TestProviderCapabilityQuarantineReleased:
    def test_creates_with_operator(self) -> None:
        event = ProviderCapabilityQuarantineReleased(mcp_server_id="rogue-provider", released_by="ops@example.com",)
        assert event.released_by == "ops@example.com"


class TestToolSchemaDriftDetected:
    def test_new_tool_added(self) -> None:
        event = ToolSchemaDriftDetected(mcp_server_id="code-runner", tools_added=["exec_shell"],
        tools_removed=[],
        tools_changed=[],)
    ...
    def test_tool_removed(self) -> None:
        event = ToolSchemaDriftDetected(mcp_server_id="math", tools_added=[],
...
class TestCapabilityDeclarationMissing:
    ...
    def test_block_mode(self) -> None:
        event = CapabilityDeclarationMissing(mcp_server_id="untrusted-server", enforcement_mode="block",)
