"""A passing health check returns a group member to rotation, on the wiring ``serve --http`` runs (#1355).

Each mode runs ``_group_recovery_harness.py`` in a fresh interpreter: the real
``bootstrap()``, then ``hangar_call`` through the served app, driving a group's
members out of rotation with consecutive failures, then the health-check worker
``bootstrap()`` created. Nothing here builds a saga or registers a member with
one. The path under test is the one a replica takes: the worker's
``HealthCheckPassed``, the event bus, the ``GroupRebalanceSaga`` that
``bootstrap()`` registered, and ``McpServerGroup.report_success()``.
