...
per-tenant pins without an identity are. Whether a hold belongs on
`resources/read` / `prompts/get` at all is decided against in ADR-024, so the
refusal is the answer rather than a placeholder
