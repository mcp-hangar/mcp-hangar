...
| `mcp-hangar pin` computes, writes and checks the digests the gate enforces (#1191) | CLI | printed digests; a written `tool_projection.pins`; exit code | `tests/live/test_t0_pin.py` (real `examples/rugpull` upstream: `pin` reports a 64-hex digest per tool, `--write` writes them plus a `.bak` of the previous file, `--check` then exits 0; changing the upstream's *description* -- no parameter touched -- makes `--check` exit **1** and print both digests; an unknown `--server` exits **2**, so "unanswerable" never reads as "clean") | ✅ |
| `init` writes a configuration that governs, and one Hangar accepts (#1192) | CLI (`init` + `config check`) | the generated file; `config check` exit code | `tests/live/test_t0_init.py` (offline `init -y --skip-clients --skip-test`: `config check` exits 0, `tool_access.mode: front_door` and `auth.stdio.principal` are present, and `--skip-test` writes **no** pins -- an unverified pin would refuse every call to a tool nobody digested). Guards the shape of the defect this replaced: a generated `health_check:` block nothing read, which `init` reported as success while every load logged `unknown_config_key`. | ✅ |
| A declared stdio principal makes `front_door` reachable, and the quickstart's sequence ends in a refusal (ADR-026 / #1189) | CLI over stdio + SDK `ClientSession` | `tools/list`; `CallToolResult.is_error` | `tests/live/test_t0_stdio_funnel.py` (the operator's path, not the developer's: the config is the one `pin --write` produced, the gateway is the console script over stdio, the caller is the SDK client. With the principal the upstream's flat `echo` is listed and callable; after the upstream changes its description the identical call comes back `is_error=True` with the pinned-digest message; the `viewer` role projects the fleet reads and nothing that changes state; **without** the block the same config serves `[]` -- the #902 fail-closed behaviour ADR-026 must not loosen) | ✅ |
...
| The epic's tracing scenarios have the right tree, outcome and bounded reason at a real OTLP receiver (#1304) | MCP `hangar_call` over `/mcp` with a tenant's `X-API-Key` + in-process OTLP/gRPC receiver; approvals resolved over `/api/approvals` by a second principal in the tenant | span parent IDs, links, `hangar.gate.decision` events, `hangar.call.outcome` / `hangar.refusal.*`, `hangar.retry.*`, `approval.result`, `/metrics` and the gateway's RSS | `tests/live/test_t3_trace_scenarios.py`: ten concurrent calls to a cold server produce one `mcp_server.launch` under the one span marked `hangar.startup.role=leader`, and each of the nine other callers has one `mcp_server.startup_wait` parented in its own trace; a retry to success (two failures, three attempts) and to exhaustion record `hangar.retry.attempt` events with index and backoff and `hangar.retry.outcome` `success` / `exhausted`; an approval granted, denied and expired each set `approval.result` to that outcome; an allowed call records only `allow`/`skip` decisions, and a `deny_list` or approval refusal names the refusing gate with a bounded reason, leaves `batch.call.<tool>` UNSET and has no cold-start, launch, retry, dispatch or CLIENT span after it; with the receiver stopped, 40 calls still succeed, RSS grows by less than 64 MiB and `mcp_hangar_otlp_export_failures_total` rises. Strict xfail: a follower that waits on the aggregate's readiness event carries no link to the start, and in this run every follower did. Not covered: inbound or outbound W3C trace context, an HTTP-layer retry, a timeout | ✅ |
